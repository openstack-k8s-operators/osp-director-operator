...
const (
	// OspDirectorOperatorWebhookLabel - Label identifying that a webhook belongs to this operator
	OspDirectorOperatorWebhookLabel string = "osp-director.openstack.org/webhook"
)

// EnsureValidationWebhooks - Creates or updates (if necessary) validation webhooks for "kind"
func EnsureValidationWebhooks(goClient client.Client, kind string) error {
	...
	for _, obj := range objs {
		obj.SetLabels(labels.Merge(obj.GetLabels(), map[string]string{
			OspDirectorOperatorWebhookLabel: "true",
		}))
